...
module("scrollTop", function(hooks) {
  ...
  test("scroll but within content length", function(assert) {
    ...
    return new RSVP.Promise(function(resolve) {
      ...
      .then(() => {
        assert.equal(findScrollable(this).prop("scrollTop"), 50, "Scrolled one row.");
      ...
      .then(() => {
        ...
        assert.equal(
          $(positionSorted[0])
            ...
            .trim(),
          "Item 1",
          "The first item is not visible but in buffer."
        );
        checkContent(this, assert, 0, 5);
      });
  });
  ...
  test("scroll within content length, beyond buffer", function(assert) {
    ...
    assert.equal(
      $(positionSorted[0])
        ...
        .trim(),
      "Item 1",
      "The first cell should be the first item."
    );
    ...
    return new RSVP.Promise(function(resolve) {
      ...
      .then(() => {
        assert.equal(findScrollable(this).prop("scrollTop"), 150, "scrolled to item 7");
        ...
        assert.equal(
          $(positionSorted[0])
            ...
            .trim(),
          "Item 7",
          "The items before what is on screen is not visible."
        );
      ...
      .then(() => {
        assert.equal(findScrollable(this).prop("scrollTop"), 50, "Scrolled down one row.");
        let positionSorted = sortItemsByPosition(this, true);
        assert.equal(
          $(positionSorted[0])
            .text()
            .trim(),
          "Item 5",
          "The fifth item is first rendered."
        );
